...
export function collectAssetRefs(data, fieldPaths) {
  ...
  const walk = (node, fields, prefix) => {
    ...
    // A list of things to look inside: numbered from 1, and walked with the fields still to go.
    // This is what a field path steps over silently — including a JSON document that is itself an
    // array, where there is no field name above the list at all.
    if (Array.isArray(node)) {
      node.forEach((item, i) => walk(item, fields, prefix ? `${prefix}-${i + 1}` : `${i + 1}`));
      return;
    }
    ...
    // Still digging: step into the field, whether it holds an object or a list of them
    if (rest.length > 0) {
