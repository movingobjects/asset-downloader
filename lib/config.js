...
/**
 * Two sources that write to the same place will overwrite each other's work, and the loser
 * is left with a JSON file pointing at assets that are no longer on disk. Caught here, up
 * front, rather than halfway through a run that has already published one of them.
 */
function checkSourcesDoNotCollide(sources) {
  const files = new Set();
  const assetDirs = new Set();

  for (const { dir, file, assets } of sources) {
    const where = dir ? `"${dir}"` : 'the top level of outDir';

    check(!files.has(`${dir}/${file}`), `Two sources both write "${file}" to ${where} — give one of them its own "dir"`);
    files.add(`${dir}/${file}`);

    if (assets.length === 0) continue; // a source with no assets never touches the assets folder

    check(!assetDirs.has(dir), `Two sources both download assets into ${where} — give one of them its own "dir"`);
    assetDirs.add(dir);
  }
}
...
/** Drops trailing slashes but keeps a leading one: "/assets" and "assets" mean different things. */
function normalizePrefix(prefix) {
  const trimmed = prefix.trim().replace(/\/+$/, '');
  return trimmed === '' && prefix.includes('/') ? '/' : trimmed;
}
