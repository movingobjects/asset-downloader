...
import { joinPath } from './assets.js';
...
export function intro(config, { jsonOnly, dryRun }) {
  ...
  line(`  on disk    ${dim(path.join(config.outDir, '<dir>'))}`);
  line(`  in json    ${dim(joinPath(config.pathPrefix, '<dir>'))}`);
...
export function created(file) {
  ...
  line(`  ${dim(file)}`);
  line();
  line(`  ${dim('Point it at your data sources, then run')} asset-sync`);
  line();
}
