...
export function written(source, hasAssets) {
  if (hasAssets) line(`  ${dim(source.assetPath)}`);
  line(`  ${dim(source.outputPath)}`);
  line();
...
export function skipped(failures, source) {
  erase();
  line();
  ...
  line(`  ${dim(`${source.outputDir} was left unchanged`)}`);
  line();
}

export function wouldWrite(jobs, source, hasAssets, { jsonOnly }) {
  if (jobs.length > 0) {
